...
// Cartographer variables
std::unique_ptr<cartographer_ros::Node> cartographer_node;
std::shared_ptr<tf2_ros::Buffer> tf_buffer;
ros::Subscriber cartographer_pose_sub;
geometry_msgs::PoseWithCovarianceStamped last_cartographer_pose;
bool has_cartographer_pose = false;
ros::ServiceClient start_trajectory_client;
bool cartographer_initialized = false;

void initializeCartographerWithGPS(const xbot_msgs::AbsolutePose& gps_pose) {
    if (cartographer_initialized) {
        return;
    }

    cartographer_ros_msgs::StartTrajectory srv;
    srv.request.options.trajectory_builder_options_proto = 
        cartographer_node->node_handle()->get_trajectory_builder_options().SerializeAsString();
    srv.request.options.tracking_frame = "base_link";
    srv.request.options.published_frame = "base_link";
    srv.request.options.odom_frame = "base_link";
    srv.request.options.provide_odom_frame = false;
    srv.request.options.use_odometry = false;
    srv.request.options.use_nav_sat = false;
    srv.request.options.use_landmarks = false;
    srv.request.options.num_laser_scans = 1;
    srv.request.options.num_multi_echo_laser_scans = 0;
    srv.request.options.num_subdivisions_per_laser_scan = 1;
    srv.request.options.num_point_clouds = 0;

    // Set initial pose from GPS
    srv.request.initial_pose.position.x = gps_pose.pose.pose.position.x;
    srv.request.initial_pose.position.y = gps_pose.pose.pose.position.y;
    srv.request.initial_pose.position.z = 0.0;  // Assuming 2D operation
    srv.request.initial_pose.orientation = gps_pose.pose.pose.orientation;

    if (start_trajectory_client.call(srv)) {
        ROS_INFO("Successfully initialized Cartographer with GPS pose");
        cartographer_initialized = true;
    } else {
        ROS_ERROR("Failed to initialize Cartographer with GPS pose");
    }
}

void onCartographerPose(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg) {
    last_cartographer_pose = *msg;
    has_cartographer_pose = true;

    // Extract position and orientation from Cartographer's pose
    ...
    // Update the Kalman filter with Cartographer's pose
    // The weight (last parameter) determines how much we trust Cartographer vs. our existing system
    // You may need to tune this value based on your specific requirements
    core.updatePosition(x, y, 10.0);
    core.updateOrientation(yaw, 10.0);
}
...
void onImu(const sensor_msgs::Imu::ConstPtr &msg) {
    ...
    if (has_cartographer_pose) {
        // Use Cartographer's pose to update the Kalman filter
        tf2::Quaternion q;
        tf2::fromMsg(last_cartographer_pose.pose.pose.orientation, q);
        tf2::Matrix3x3 m(q);
        double roll, pitch, yaw;
        m.getRPY(roll, pitch, yaw);

        core.updatePosition(last_cartographer_pose.pose.pose.position.x, 
                            last_cartographer_pose.pose.pose.position.y, 
                            5.0);  // Weight for Cartographer position update
        core.updateOrientation(yaw, 5.0);  // Weight for Cartographer orientation update
    }
    ...
    map_trans.header = odometry.header;
    map_trans.child_frame_id = odometry.child_frame_id;
    map_trans.transform.translation.x = odometry.pose.pose.position.x;
    map_trans.transform.translation.y = odometry.pose.pose.position.y;
    map_trans.transform.translation.z = odometry.pose.pose.position.z;
    map_trans.transform.rotation = odometry.pose.pose.orientation;

    static tf2_ros::TransformBroadcaster transform_broadcaster;
    transform_broadcaster.sendTransform(map_trans);
    ...
    // Add Cartographer update
    cartographer_node->HandleImuMessage(msg->header.frame_id, msg);
...
void onPose(const xbot_msgs::AbsolutePose::ConstPtr &msg) {
    ...
    double time_since_last_gps = (ros::Time::now() - last_gps_time).toSec();
        ...
        if (distance_to_last_gps < 5.0) {
            ...
            if (!has_gps && valid_gps_samples > 10) {
                ...
                // Initialize Cartographer with the first valid GPS pose
                initializeCartographerWithGPS(*msg);
            } else if (has_gps) {
    ...
    void onLaserScan(const sensor_msgs::LaserScan::ConstPtr &msg) {
        if (cartographer_initialized) {
            cartographer_node->HandleLaserScanMessage(msg->header.frame_id, msg);
        }
    }
    ...
    int main(int argc, char **argv) {
        ...
        ros::Subscriber laser_scan_sub = paramNh.subscribe("scan", 10, onLaserScan);
    
        // Initialize Cartographer
        auto node_options = cartographer_ros::LoadOptions();
        tf_buffer = std::make_shared<tf2_ros::Buffer>(ros::Duration(tf2::BufferCore::DEFAULT_CACHE_TIME));
        auto tf_listener = std::make_shared<tf2_ros::TransformListener>(*tf_buffer);
        cartographer_node = cartographer_ros::Node::Make(node_options, tf_buffer);
    
        start_trajectory_client = n.serviceClient<cartographer_ros_msgs::StartTrajectory>("/start_trajectory");
    
        cartographer_pose_sub = n.subscribe("cartographer/pose", 10, onCartographerPose);
